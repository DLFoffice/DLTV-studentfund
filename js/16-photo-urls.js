...
(function(){
  ...
  // ── ชุด URL จาก Excel (ไว้ตรวจว่ารูปไหน "มาจากการ assign อัตโนมัติ") ──
  const excelSet = new Set(photoUrlsFromExcel);

  // รูปที่ถูกต้องของนักเรียน = ตำแหน่งตาม "ลำดับ" (no) ไม่ใช่ตำแหน่งในอาร์เรย์
  // Excel เรียงตามลำดับ 1..72 → รูปคนที่ no=k คือ photoUrlsFromExcel[k-1]
  function correctPhotoFor(s) {
    const n = Number(s && s.no);
    if (!Number.isInteger(n) || n < 1 || n > photoUrlsFromExcel.length) return null;
    return photoUrlsFromExcel[n - 1];
  }

  /* จับคู่รูปกับนักเรียนตาม "ลำดับ" — idempotent (เรียกซ้ำได้ ปลอดภัย)
     - คนที่ยังไม่มีรูป → ใส่รูปตามลำดับ
     - คนที่รูปปัจจุบัน "มาจากชุด Excel" แต่ผิดตำแหน่ง → แก้ให้ตรงลำดับ
     - รูปที่อัปโหลดเอง (base64/ลิงก์อื่น) จะไม่ถูกแตะ
     คืนค่า true ถ้ามีการเปลี่ยนแปลงจริง */
  window.applyExcelPhotosByNo = function applyExcelPhotosByNo() {
    if (typeof DB === 'undefined' || !Array.isArray(DB.students)) return false;
    let changed = false;
    DB.students.forEach(s => {
      const correct = correctPhotoFor(s);
      if (!correct) return;
      const cur = s.photoUrl || '';
      const isAutoAssigned = cur === '' || excelSet.has(cur);   // ว่าง หรือเป็นรูปชุด Excel
      if (isAutoAssigned && cur !== correct) { s.photoUrl = correct; changed = true; }
    });
    return changed;
  };

  // เรียกตอนโหลดหน้า (เผื่อข้อมูลอยู่ในเครื่องแล้ว) + ทุกครั้งที่วาดรายชื่อ/แดชบอร์ด/GPA
  // (ข้อมูลจากคลาวด์/Sheet เข้ามาทีหลัง จึงต้องตรวจซ้ำ — แต่จะบันทึกเฉพาะรอบที่แก้จริง)
  function runAndPersist() {
    try {
      if (window.applyExcelPhotosByNo()) {
        try { saveToStorage(); } catch (e) {}
      }
    } catch (e) { console.warn('applyExcelPhotosByNo:', e); }
  }
  ['renderStudents', 'renderDashboard', 'renderGpaSheet'].forEach(fn => {
    const orig = window[fn];
    if (typeof orig !== 'function') return;
    window[fn] = function () { runAndPersist(); return orig.apply(this, arguments); };
  });
  runAndPersist();
  console.log('✅ จับคู่รูปภาพตาม "ลำดับ" เรียบร้อย (แก้ปัญหารูปไม่ตรงชื่อ)');
})();
