...
(function () {
  ...
  function sdqEditorHtml() {
    ...
    const itemsHtml = SDQ_ITEMS.map(it => {
      const cur = data.answers[it.id];
      return `<tr>
        <td style="width:34px;text-align:center;color:var(--text3)">${it.id}</td>
        <td>${sdqEsc(it.text)}</td>
        ${CHOICES.map((c, i) => `<td style="text-align:center;width:80px">
  ...
  function sdqRefreshResultInline(host, data) {
    ...
    box.querySelectorAll('.sdq-result-card').forEach((card, i) => {
      ...
      card.querySelector('.sdq-result-card-val').textContent = res.domainScore[d];
  ...
  function sdqBuildPrintHtml(student, data, res) {
    ...
    <div class="sfp-page">
      ...
      <table class="sfp-table" style="margin-top:12px">
        <thead><tr><th style="width:26px"></th><th style="text-align:left">รายการประเมิน</th>${CHOICES.map(c => `<th>${c}</th>`).join('')}</tr></thead>
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .sdq-banner{padding:10px 16px;border-radius:var(--rad);margin-bottom:14px;font-size:13px;font-weight:600}
      ...
      .sdq-topfields input{padding:8px 10px;border-radius:var(--rad);border:1px solid var(--border2);font-family:'Sarabun',sans-serif;font-size:13px}
      .sdq-table{width:100%;border-collapse:collapse;font-size:13px;background:var(--bg4);border:1px solid var(--border);border-radius:var(--rad-lg);overflow:hidden;margin-bottom:18px}
      .sdq-table th,.sdq-table td{padding:8px 10px;border-bottom:1px solid var(--border)}
      .sdq-table thead th{background:var(--bg2);font-size:11px;color:var(--text2);text-transform:uppercase;letter-spacing:.4px}
      .sdq-table tbody tr:hover{background:var(--blue-lt)}
      .sdq-result-box,.sdq-impact-box{background:var(--bg4);border:1px solid var(--border);border-radius:var(--rad-lg);padding:16px;margin-bottom:16px}
      ...
      .sdq-result-card{background:var(--bg2);border-radius:var(--rad);padding:10px;text-align:center}
      .sdq-result-card-lbl{font-size:11px;color:var(--text3);margin-bottom:4px}
      .sdq-result-card-val{font-size:22px;font-weight:800;margin-bottom:4px}
      .sdq-impact-box label{display:block;margin-bottom:10px;font-size:13px;font-weight:600;color:var(--text2)}
      .sdq-impact-box select{margin-top:4px;padding:7px 10px;border-radius:var(--rad);border:1px solid var(--border2);font-family:'Sarabun',sans-serif;font-size:13px;width:100%;max-width:360px}
