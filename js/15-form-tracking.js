...
function ftOverallStatus(student){
  const {has1, has2} = sfFormStatus(student);
  if(has1 && has2) return 'done';
  if(has1 || has2) return 'partial';
  return 'none';
}
...
function renderFormTrack(){
  ...
  document.getElementById('formtrack-tbody').innerHTML = slice.map(s=>{
    ...
    const {has1, has2} = sfFormStatus(s);
    const status = ftOverallStatus(s);
    const meta = ftStatusMeta(status);
    const orderNo = s.no || (idx+1);
    const previewLinks = (has1||has2) ? `<div style="display:flex;gap:6px;margin-top:4px;font-size:11px">
        ${has1?`<a href="javascript:void(0)" onclick="ftOpenPreview(${idx},'form1')" style="color:var(--blue);text-decoration:underline">👁️ พรีวิวฟอร์ม 1</a>`:''}
        ${has2?`<a href="javascript:void(0)" onclick="ftOpenPreview(${idx},'form2')" style="color:var(--blue);text-decoration:underline">👁️ พรีวิวฟอร์ม 2</a>`:''}
      </div>` : '';
    return `<tr>
    ...
    return `<button type="button" class="sf-card" style="animation-delay:${delay}s" data-sf-action="open-student" data-sf-idx="${idx}">
      ...
      <span class="sf-card-body">
        ...
        <span class="sf-card-status">
          <span class="sf-tag ${st.has1?'done':''}">${st.has1?'✓ ':''}แบบฟอร์ม 1</span>
          <span class="sf-tag ${st.has2?'done':''}">${st.has2?'✓ ':''}แบบฟอร์ม 2</span>
        </span>
      </span>
    </button>`;
  }).join('');
...
function sfPersist(){
  ...
  if(student){
    ...
    student[sfState.formKey].__touched = true;
  }
  _sfDebouncedSave();
}
...
async function sfSendToSheet(btn){
  ...
  if(typeof SCRIPT_URL==='undefined' || !SCRIPT_URL){
    showStatus('⚠️ ยังไม่ได้ตั้งค่า SCRIPT_URL ในระบบ', 'error');
    return;
  }
  ...
  try{
    ...
    showStatus('☁️ ส่ง'+formLabel+' ของ '+(student.name||'นักเรียน')+' ไปยัง Google Sheet แล้ว'+(result&&result.sheetName?' (ชีต: '+result.sheetName+')':''), 'success');
  } catch(err){
