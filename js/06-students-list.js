  ...
  if(currentView==='card'){
    document.getElementById('students-card-grid').innerHTML=filtered.map(s=>{
      ...
      return `<div class="student-card" onclick="openStudentDetail(${idx})">
        <div class="card-photo-wrap">
          ...
          ${(typeof sdqRiskBadge==='function'&&sdqRiskBadge(s))
            ? `<div class="card-risk-badge">${sdqRiskBadge(s)}</div>`
    ...
    document.getElementById('students-tbody').innerHTML=slice.map(s=>{
      ...
      return`<tr>
        ...
        <td>${(typeof sdqRiskBadge==='function'&&sdqRiskBadge(s)) ? sdqRiskBadge(s) : `<span class="${riskBadge(g.riskLevel)}">${g.riskLevel||'-'}</span>`}</td>
