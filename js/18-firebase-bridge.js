...
(function () {
  ...
  function applyCloudStudents(students) {
    ...
    if (studentMode && window.STUDENT_MODE) {
      ...
      const filtered = normalized.filter(s => String(s.id) === myId);
  ...
  auth.onAuthStateChanged(async user => {
    ...
    try {
      if (studentMode) {
        ...
        let sid = String(accData.studentId);
        let d = await fsdb.collection(COL).doc(sid).get();

        // ── ซ่อมการเชื่อมอัตโนมัติ: ถ้าเอกสารตาม studentId ไม่มีอยู่
        //    (เช่น เลขบัตรถูกแก้ / ลบ-เพิ่มนักเรียนใหม่หลังสมัคร)
        //    → ค้นหาระเบียนของตัวเองจาก "ลำดับ" แล้วผูกบัญชีใหม่ให้เอง ──
        if (!d.exists && accData.no != null) {
          try {
            const q = await fsdb.collection(COL).where('no', '==', accData.no).limit(1).get();
            if (!q.empty) {
              const newSid = q.docs[0].id;
              await fsdb.collection('accounts').doc(user.uid)
                .set({ studentId: newSid }, { merge: true });
              accData.studentId = newSid;
              if (window.STUDENT_MODE) window.STUDENT_MODE.studentId = newSid;
              sid = newSid;
              d = q.docs[0];
              console.log('☁️ ซ่อมการเชื่อมบัญชีอัตโนมัติ: studentId →', newSid);
            }
          } catch (e) {
            console.warn('☁️ ซ่อมการเชื่อมอัตโนมัติไม่สำเร็จ:', e.code, e.message,
              '(ถ้าเป็น permission-denied แปลว่า Rules ยังไม่ใช่เวอร์ชันล่าสุด)');
          }
        }

        // ── กรณีเอกสารมีอยู่ แต่ "เลขบัตรในข้อมูล" ไม่ตรงกับรหัสเอกสาร ──
        //    (เกิดเมื่อครูแก้เลขบัตรนักเรียนหลังจากผูกบัญชีไว้แล้ว)
        //    → ย้ายการผูกบัญชีไปเลขใหม่ + ลบเอกสารค้างเก่า อัตโนมัติ
        if (d.exists) {
          const trueId = String((d.data() && d.data().id) || sid);
          if (trueId && trueId !== 'undefined' && trueId !== sid) {
            try {
              // อ่านเอกสารเลขใหม่ (Rules อนุญาตเพราะ "ลำดับ" ตรงกับบัญชีเรา)
              const realDoc = await fsdb.collection(COL).doc(trueId).get();
              const carry = JSON.parse(JSON.stringify(realDoc.exists ? realDoc.data() : d.data()));
              await fsdb.collection(COL).doc(sid).delete();          // ลบเอกสารค้าง (ตอนนี้ยังมีสิทธิ์)
              await fsdb.collection('accounts').doc(user.uid)
                .set({ studentId: trueId }, { merge: true });        // ผูกบัญชีกับเลขใหม่
              accData.studentId = trueId;
              if (window.STUDENT_MODE) window.STUDENT_MODE.studentId = trueId;
              if (!realDoc.exists)                                   // ถ้าเลขใหม่ยังไม่มีเอกสาร ให้ย้ายข้อมูลไป
                await fsdb.collection(COL).doc(trueId).set(carry, { merge: false });
              sid = trueId;
              d = { exists: true, data: () => carry };
              console.log('☁️ ย้ายการผูกบัญชีตามเลขบัตรใหม่:', trueId, '(ลบเอกสารค้างเรียบร้อย)');
            } catch (e) {
              console.warn('☁️ ย้ายการผูกบัญชีไม่สำเร็จ:', e.code, e.message);
            }
          }
        }
        ...
        if (typeof setSyncDot === 'function') setSyncDot('online');
        if (!d.exists)
          showTopBanner('⚠️ ไม่พบข้อมูลของคุณในระบบ (บัญชีผูกกับ studentId: ' + sid
            + ' ลำดับ: ' + (accData.no ?? '-') + ') — แจ้งครูผู้ดูแลให้เปิดหน้า '
            + '"จัดการบัญชีผู้ใช้" แล้วกดปุ่ม "🔧 ซ่อมการเชื่อม" หรือตรวจว่า Publish Rules เวอร์ชันล่าสุดแล้ว');
