...
(function () {
  ...
  function showTopBanner(msg) {
    ...
    if (!b) {
      ...
      b.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99997;background:#FEF3C7;'
        ...
        + "font-family:'Noto Sans Thai',sans-serif;font-size:13.5px;line-height:1.6;text-align:center";
  ...
  function applyStudentRestrictions() {
    ...
    // เมนู "พฤติกรรมนักเรียน" (SDQ) ถูกซ่อนไปพร้อม section label ด้านบน — เปิดกลับมาเฉพาะ label ที่คุมปุ่ม SDQ
    document.querySelectorAll('#sidebar-nav .sidebar-section-label').forEach(l => {
      if ((l.textContent || '').indexOf('พฤติกรรมนักเรียน') !== -1) l.style.display = '';
    });
    // ซ่อนปุ่ม Sync/Save/Export (งานของครู/แอดมิน)
    const acts = document.querySelector('.sidebar-actions');
    if (acts) acts.style.display = 'none';
    // พาเข้าหน้าแบบฟอร์มของตัวเองทันที — แต่ "เฉพาะตอนยังไม่อยู่ในหน้าที่อนุญาต" เท่านั้น
    // (ฟังก์ชันนี้ถูกเรียกซ้ำอีกครั้งหลัง Auth ยืนยันสิทธิ์จริงจากคลาวด์ — ถ้าบังคับ
    //  showPage('scholarform') ทุกครั้งแบบไม่มีเงื่อนไข จะทำให้นักเรียนที่เพิ่งกดเข้าไปดู
    //  "แบบประเมิน SDQ" หรือหน้าอื่นที่อนุญาตไว้ ถูกเด้งกลับมาหน้าแบบฟอร์มทุนโดยไม่ตั้งใจ)
    const allowedPages = ['page-scholarform', 'page-formtrack', 'page-sdqform', 'page-sdqdashboard'];
    const activePage = document.querySelector('.page.active');
    if (!activePage || allowedPages.indexOf(activePage.id) === -1) {
  ...
  ui.innerHTML = `
    <style>
      ...
      .fb-float{position:fixed;bottom:14px;right:14px;z-index:9000;display:none;flex-direction:column;
        align-items:flex-end;gap:8px;font-family:'Noto Sans Thai',sans-serif}
      .fb-float button{border:none;border-radius:20px;padding:8px 15px;font-size:12.5px;cursor:pointer;
        font-family:'Noto Sans Thai',sans-serif;box-shadow:0 2px 8px rgba(0,0,0,.25);color:#fff;background:rgba(26,29,46,.9)}
      #fb-pass-modal{position:fixed;inset:0;background:rgba(20,18,10,.5);z-index:99998;display:none;
        align-items:center;justify-content:center;font-family:'Noto Sans Thai',sans-serif}
      ...
      .fb-field input{width:100%;padding:11px 40px 11px 12px;border:1.5px solid #E5E0D4;border-radius:10px;
        font-size:14px;font-family:'Noto Sans Thai',sans-serif;box-sizing:border-box}
