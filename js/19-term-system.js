...
(function () {
  ...
  function _isUserEntered(student, store, f) {
    ...
    if (f.type === 'table') {
      return Array.isArray(v) && v.some(r => r && r.cells && Object.values(r.cells).some(x => x !== '' && x != null));
    }
    if (f.type === 'matrix') {
  ...
  /* ══════════ กันนักเรียนกรอกแบบฟอร์มที่ถูกปิด ══════════ */
  function applyFormAccessGate() {
    const root = document.getElementById('sf-root'); if (!root) return;
    if (isStaffUser()) return;                       // ครู/แอดมินไม่ถูกจำกัด
    const key = (typeof sfState === 'object') ? sfState.formKey : 'form1';
    if (isFormOpen(key)) return;
    ...
    if (main) {
      ...
      main.querySelectorAll('[data-sf-action="add-row"],[data-sf-action="del-row"],[data-sf-action="add-col"]')
        .forEach(b => { b.disabled = true; });   // ปิดเฉพาะปุ่มแก้ข้อมูล (ยังเลื่อนดูส่วนอื่นได้)
      if (!main.querySelector('.sf-closed-note')) {
        const note = document.createElement('div');
        note.className = 'sf-closed-note';
        note.innerHTML = '🔒 แบบฟอร์มนี้ถูกปิดรับการกรอกโดยผู้ดูแลระบบ — ขณะนี้ยังกรอกหรือบันทึกไม่ได้ กรุณารอการเปิดสิทธิ์';
        main.prepend(note);
      }
    }
