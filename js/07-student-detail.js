          ...
          <div class="photo-section">
            ...
            <div class="photo-controls">
              ...
              <div id="photo-src-url-${idx}" style="display:none">
                <input style="width:100%;padding:8px 10px;border-radius:var(--rad);border:1px solid var(--border2);font-size:12px;font-family:'Noto Sans Thai',sans-serif" 
                  value="${s.photoUrl||''}" 
...
function renderBanking(){
  ...
  document.getElementById('banking-tbody').innerHTML=filtered.map(s=>{
    ...
    return`<tr><td>${s.no}</td><td class="photo-cell">${photoEl(s)}</td><td style="font-weight:600">${s.name}<div style="font-size:11px;color:var(--text3)">${s.nickname||''}</div></td>
      <td style="font-family:'Noto Sans Thai',monospace;font-size:11px">${b.contract||'-'}</td>
      ...
      <td style="font-family:'Noto Sans Thai',monospace;font-size:12px">${b.accNoSt||'-'}</td>
      <td><span class="badge b-teal">${b.bankSch||'-'}</span></td>
      <td style="font-size:12px">${b.branchSch||'-'}</td>
      <td style="font-family:'Noto Sans Thai',monospace;font-size:12px">${b.accNoSch||'-'}</td>
      <td><button class="btn btn-sm" onclick="openStudentDetail(${idx})">แก้ไข</button></td></tr>`;
  }).join('');
...
// สีป้ายผลประเมิน SDQ รวม 4 ด้าน (ปกติ=เขียว / เสี่ยง=เหลือง / มีปัญหา=แดง)
function sdqGroupBadge(g){
